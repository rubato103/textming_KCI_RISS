...
# 변수 초기화를 먼저 수행
available_memory_gb <- 8  # 기본값
available_memory_gb <- tryCatch({
  # 방법 1: wmic 명령어로 사용 가능 메모리 확인
  mem_info <- system('wmic OS get FreePhysicalMemory /value', intern = TRUE, ignore.stdout = TRUE, ignore.stderr = TRUE)
...
if (use_cores <= 2 || total_docs <= 50) {
  ...
  tryCatch({
    # 모든 워커 메시지 억제
    suppressMessages({
      cl <- makeCluster(use_cores, type = "PSOCK", outfile = "")
    })
