...
if (nrow(all_combined) > 0) {
  ...
  if (nrow(dict_candidates) > 0) {
    # 복합명사 후보는 이미 위에서 combined_csv_file로 저장됨
    cat(sprintf("✅ 복합명사 후보 필터링 완료: %d개 (위 파일에 포함)\n", nrow(dict_candidates)))
    ...
    cat("⚠️ 필터링 조건을 만족하는 복합명사 후보가 없습니다.\n")
  }
}
...
for (n in NGRAM_SIZES) {
  ngram_data <- all_ngram_results[[paste0(n, "gram")]]
  if (!is.null(ngram_data) && is.data.frame(ngram_data) && nrow(ngram_data) > 0) {
    report_text <- paste0(report_text, sprintf("- `%dgram_results_%s.csv`: %d개\n", 
                                              n, date_suffix, nrow(ngram_data)))
  }
}
if (nrow(all_combined) > 0) {
  report_text <- paste0(report_text, sprintf("- `compound_nouns_candidates_%s.csv`: %d개 (복합명사 후보)\n", 
                                            date_suffix, nrow(all_combined)))
}
report_text <- paste0(report_text, sprintf("- `proper_nouns_candidates_%s.csv`: 고유명사 후보 (빈 템플릿)\n", 
                                          date_suffix))

report_text <- paste0(report_text, "\n## 다음 단계\n\n")
report_text <- paste0(report_text, "1. Excel에서 생성된 CSV 파일을 열어 검토\n")
report_text <- paste0(report_text, "2. 등록하지 않을 단어의 행을 삭제\n")
report_text <- paste0(report_text, "3. 고유명사 후보 파일에 필요한 고유명사 직접 추가\n")
...
cat("4. 03-1_register_user_dict.R 실행하여 사전 등록\n")
